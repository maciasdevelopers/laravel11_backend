...
class VHUM_SolicitudesCancelacionController extends Controller{
  //funciones para solicitudes de cancelación realizadas
  /*
    CREATE TABLE IF NOT EXISTS vhum_solicitudes_cancelacion (
      id INT(10) primary key NOT NULL AUTO_INCREMENT,
      
      -- Identificadores y Folios
      token_soli_cancel VARCHAR(255),
      folio_soli_cancel INT(8) UNSIGNED,
      fecha_soli_cancel INT(10) UNSIGNED NOT NULL COMMENT 'Fecha Unix en que se hace la solicitud',
      vhum_movimiento_fecha_cont INT(10) UNSIGNED NOT NULL COMMENT 'Fecha Unix de contabilización en que se hace la solicitud',
      vhum_movimiento_tipo ENUM('nomina', 'isn', 'imss', 'trabajadores') NOT NULL,
      vhum_movimiento_id int(10) COMMENT 'ID del registro relacionado',
    
      -- Observaciones y Justificación
      vhum_observaciones_mov TEXT DEFAULT NULL COMMENT 'Comentarios o justificación del movimiento',
    
      -- Claves foráneas (Relaciones)
      vhum_empresa INT(10) NOT NULL COMMENT 'ID de la empresa vinculada',
    
      -- Banderas de control y Estado
      vhum_cancel_realizada boolean DEFAULT 0 COMMENT '0: Pendiente, 1: Procesado / Aplicado',
      vhum_cancel_status boolean DEFAULT 1 COMMENT '1: Activo, 0: Cancelado/Rechazado',
      vhum_cancel_fecha_delete INT(10) UNSIGNED DEFAULT NULL COMMENT 'Timestamp de eliminación lógica',
    
      -- Llaves Primaria e Índices
      foreign key (vhum_empresa) references main_empresas (id)
    );
  */
    ...
    if ($validate->fails()) {
      ...
      $soliCancelVHUM = DB::table("vhum_solicitudes_cancelacion AS vhSCanc")
      ->join("main_empresas AS emp","vhSCanc.vhum_empresa","emp.id")
      ...
      ->when($periodo != 'all_partidas', function ($query) use ($fechaInicio, $fechaFin) {
        return $query->whereBetween("vhSCanc.vhum_movimiento_fecha_cont", [$fechaInicio, $fechaFin]);
      })
      ->where([
        "vhSCanc.vhum_cancel_status" => TRUE,
        "emp.empresa_token" => $empresa,
        "users.usuario_token" => $usuario
      ])
      ->select([
        "vhSCanc.token_soli_cancel AS token_soli",
        "vhSCanc.folio_soli_cancel AS folio_soli",
        "vhSCanc.vhum_movimiento_tipo AS tipo_solicitud",
        "vhSCanc.vhum_movimiento_id AS doc_anterior",
        "vhSCanc.vhum_movimiento_fecha_cont AS fecha_contabilizacion",
        "vhSCanc.vhum_observaciones_mov AS observaciones",
        "vhSCanc.vhum_cancel_realizada AS cancel_realizada"
      ])
      ->orderBy("vhSCanc.fecha_soli_cancel", "desc")
      ->get();

      if ($soliCancelVHUM->isEmpty()) {
        $dataMensaje = array(
          'code' => 200,
          'status' => 'error',
        ...
        $idDocAnterior = $soliCancelVHUM->pluck('doc_anterior')->filter()->unique()->toArray();
        $nominasMainMap = DB::table('vhum_nominas_main')->whereIn('id',$idDocAnterior)->get()->keyBy('id');
        $nominasImpuestosMap = DB::table('vhum_nominas_impuestos')->whereIn('id',$idDocAnterior)->get()->keyBy('id');
        $socialMainMap = DB::table("vhum_aportaciones_seguridad_social_main")->whereIn('id',$idDocAnterior)->get()->keyBy('id');
        $empleadosMap = DB::table("vhum_empleados_catalogo")->whereIn('id',$idDocAnterior)->get()->keyBy('id');

        foreach ($soliCancelVHUM as $cSoli) {
          $solicitud_folio = 'VHUM-SOLI-CANC-'.$JwtAuth->generarFolio($cSoli->folio_soli);

          $nomina = $nominasMainMap->get($cSoli->doc_anterior);
          $isn = $nominasImpuestosMap->get($cSoli->doc_anterior);
          $imss = $socialMainMap->get($cSoli->doc_anterior);
          $empleados = $empleadosMap->get($cSoli->doc_anterior);
          
          $doc_anterior_token = match ($cSoli->tipo_solicitud) {
            'nomina' => $nomina->token_nominas_periodos,
            'isn' => $isn->nomi_imp_token,
            'imss' => $imss->aport_ssocial_token,
            'trabajadores' => $empleados->empleado_token,
            default => "",
          };

          $doc_anterior_folio = match ($cSoli->tipo_solicitud) {
            'nomina' => 'NOM-EF-'.$JwtAuth->generarFolio($nomina->nomina_folio_interior).(!is_null($nomina->nomina_subfolio) ? '-'.$nomina->nomina_subfolio : ''),
            'isn' => 'NOM-IMP-'.$JwtAuth->generarFolio($isn->nomi_imp_folio_interior).(!is_null($isn->nomi_imp_subfolio)? '-'.$isn->nomi_imp_subfolio : ''),
            'imss' => 'APORT-IMSS-'.$JwtAuth->generarFolio($imss->aport_ssocial_folio_interior).(!is_null($imss->aport_ssocial_subfolio) ? '-'.$imss->aport_ssocial_subfolio : ''),
            'trabajadores' => 'TRAB-'.$JwtAuth->generarFolio($empleados->folio_pers).(!is_null($empleados->post_folio_pers) ? '-'.$empleados->post_folio_pers : ''),
            default => "",
          };
