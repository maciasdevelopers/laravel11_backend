...
@Component({
  selector: 'app_interno_egresos_compras_por_autorizar',
  standalone: false,
  templateUrl: './compras-por-autorizar.html',
  styleUrls: [
    ...
    '../../../../../../styles/switches.css',
    '../../../egresos.css',
    './compras-por-autorizar.css'
  ]
})
export class ComprasPorAutorizar implements OnInit, OnDestroy {
  //compras por autorizar
  searchComprasNotAutorized:any = [];
  compnotAutorizedArray:any = [];
  indicadorComprasNotAutorized:'hoy'|'esta_semana'|'este_mes'|'mes_anterior'|'otras_fechas'|'all_partidas' = 'hoy';
  rangoPeriodoComprasNotAutorized: Date[] | undefined;
  loading = false;
  ...
  ngOnInit(): void {
    if (this.compnotAutorizedArray.length === 0) this.verComprasPorAtuth('hoy');
    
    this.searchComprasNotAutorized = ['token_compras','folio_compra','fecha_contabilizacion','proveedor_folio','proveedor_nombre','proveedor_nombre_comercial','compra_a_credito',
  ...
  listaComprasPorAtuth() {
    this.verComprasPorAtuth(this.indicadorComprasNotAutorized);
  }

  //compras por autorizar
  verComprasPorAtuth(filtro: 'hoy'|'esta_semana'|'este_mes'|'mes_anterior'|'otras_fechas'|'all_partidas'){
    this.indicadorComprasNotAutorized = filtro;
    let periodo_inicio = '';
    let periodo_fin = '';
    this.loading = true;
    ...
    if (filtro == 'otras_fechas') {
      ...
      if (this.rangoPeriodoComprasNotAutorized && this.rangoPeriodoComprasNotAutorized.length === 2) {
        const dateInicio = this.rangoPeriodoComprasNotAutorized[0];
        const dateFin = this.rangoPeriodoComprasNotAutorized[1];
    ...
    this._comprServ.listaComprasNoAturizadas(filtro,periodo_inicio,periodo_fin).pipe(takeUntil(this.destruir$)).subscribe({
  ...
  private procesarRespuesta(response: any) {
    ...
    if (response.status === 'success') {
      this.compnotAutorizedArray = response.compras;
      this.cd.detectChanges(); // Forzamos detección de cambios si es necesario
    } else {
      this.compnotAutorizedArray = []; // O manejar mensaje de "sin datos"
    }
  }
