...
@Component({
  selector: 'app_interno_egresos_compras_sin_factura_recibida',
  standalone: false,
  templateUrl: './recepcion-de-facturas.html',
  styleUrls: [
    ...
    '../../../../../../styles/switches.css',
    '../../../egresos.css',
    './recepcion-de-facturas.css'
  ]
})
export class RecepcionDeFacturas implements OnInit, OnDestroy {
  searchComprasSinFactura:any = [];
  listaComprasSinFactura:any = [];
  indicadorCompras:'hoy'|'esta_semana'|'este_mes'|'mes_anterior'|'otras_fechas'|'all_partidas' = 'hoy';
  ...
  ngOnInit(): void {
    if (this.listaComprasSinFactura.length === 0) this.verComprasRecibeFacturaDespues('hoy');

    this.searchComprasSinFactura = ['token_compras','folio_compra','fecha_contabilizacion','proveedor_folio','proveedor_nombre','proveedor_nombre_comercial','compra_a_credito',
  ...
  listaComprasRecibeFacturaDespues() {
    this.verComprasRecibeFacturaDespues(this.indicadorCompras);
  }

  verComprasRecibeFacturaDespues(filtro: 'hoy'|'esta_semana'|'este_mes'|'mes_anterior'|'otras_fechas'|'all_partidas'){
    this.indicadorCompras = filtro;
    let periodo_inicio = '';
    let periodo_fin = '';
    this.loading = true;
    ...
    this._comprServ.listaComprasRecibeFacturaDespues(filtro,periodo_inicio,periodo_fin).pipe(takeUntil(this.destruir$)).subscribe({
  ...
  private procesarRespuesta(response: any) {
    ...
    if (response.status === 'success') {
      this.listaComprasSinFactura = response.compras;
      this.cd.detectChanges(); // Forzamos detección de cambios si es necesario
    } else {
      this.listaComprasSinFactura = []; // O manejar mensaje de "sin datos"
    }
  }
        ...
        this._comprServ.registraOrdenRecepcionCompra(token_compras,token_cat_proveedores).subscribe(
          response => {
            ...
            if (response.status == 'success') {
              ...
              this.listaComprasRecibeFacturaDespues();
            }
