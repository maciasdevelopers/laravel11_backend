...
@Component({
  selector: 'app_orden_pago_para_compras',
  templateUrl: './pago_orden_for_compra.component.html',
  standalone: false,
  styleUrls: [
    ...
    '../../../../../../styles/switches.css',
    '../../../../../../styles/totales.css',
    './pago_orden_for_compra.component.css',
  ]
})
export class PagoOrdenForCompraComponent implements OnInit {
  ...
  ordenes_pago_for_compra_prv_token: string = "";
  ordenes_pago_for_compra_prv_name: string = "";
  ...
  ngOnInit(): void {
    ...
    this.searchPagolistaProceso = ['id', 'folio_ordenPago', 'orden_emisor_personal_nombre', 'orden_emisor_emp', 'factura_relacionada_string', 'importe_total_inicial', 'importe_autorizado_inicial_format','importe_autorizado_final', 'importe_restante_format'];
  }

  getRespuestaRegistroBuy() {
    this.relInterna.mensajeCompraPagar$.subscribe(
      (mensaje: any) => {
        if (mensaje == "nuevo_registro") {
          console.log(mensaje)
          this.relInterna.token_compras$.subscribe(
            (json_content: any) => {
              this.monedas_lista();
              ...
              this.orden_de_pago_para_compra(data.token_compras, data.token_ordenPago);
              this.ordenes_pago_for_compra_prv_token = data.token_proveedor;
              this.listar_anticipos_proveedor(data.token_proveedor);
              this.listar_saldos_a_favor_proveedor(data.token_proveedor);
            }
          );
        }
      }
  ...
  orden_de_pago_para_compra(token_compras: any, token_ordenPago: any) {
    this.ordenPago.listaordenespagoparacompras(token_compras, token_ordenPago).subscribe(
      response => {
        console.log(response);
        if (response.status == 'success') {
          this.ordenes_pago_for_compra = response.orden_generada;
        }
      },
      error => {
        console.log(error);
  ...
  ordenes_pago_adeudo() {
    ...
    this.ordenes_pago_for_compra.forEach((row: any) => {
  ...
  get sumaTotalAnticipo(): string {
    return this.ordenes_pago_anticipo_total.toLocaleString('es-MX', { style: 'currency', currency: 'MXN' });
  }

  select_fecha_contabilizacion(event: any): void {
    const validacion_xml = event.value != "" && this.validator.filtroFecha(event.value);
    this.pay_order_fecha_contabilizacion = validacion_xml ? event.value : '';
  ...
  importeByMonedero(event: any, token_cuentaMon: any) {
    ...
    const validacion = token_cuentaMon != "" && typeof moned !== 'undefined';
    moned.select_for_pagos = validacion ? event.checked : false;
    moned.monto_aplicar = validacion && moned.saldofloat < this.ordenes_pago_adeudo() ? moned.saldofloat : 0;
    console.log(moned);
    this.aumentaImporteAll();
  }
  ...
  keyupOrdenPImportePorPagar(event: any, token_ordenPago: any) {
    let ord = this.ordenes_pago_for_compra.find((row: any) => row.token_ordenPago === token_ordenPago);
  ...
  aumentaImporteAll() {
    ...
    if (this.ordenes_pago_for_compra.length == 1) {
      let ord = this.ordenes_pago_for_compra[0];
  ...
  calculaTotalPagando() {//ordenes_pago_for_compra
    var importe_autorizado = 0;
    var moneda_autorizada = "";
    var moneda_autorizada_decimales = "";
  ...
  get pay_to_validate(): Boolean {
    ...
    const pagar_list = this.ordenes_pago_for_compra.filter((row: any) => row.importe_por_pagar != "0.00");
    if (pagar_list.length == this.ordenes_pago_for_compra.length) {
      this.ordenes_pago_for_compra.forEach((row: any) => {
        importe_registrado_por_pagar += parseFloat(row.importe_por_pagar);
      });
    }
    const validacion_pagando = pagar_list.length == this.ordenes_pago_for_compra.length && importe_registrado_por_pagar > 0 && this.pay_order_importe >= importe_registrado_por_pagar;
  ...
  limpiaSeccionPagos() {
    this.ordenes_pago_for_compra = [];
    this.ordenes_pago_for_compra_prv_token = "";
    this.ordenes_pago_for_compra_prv_name = "";
    this.ordenes_pago_listaProceso_prv_saldos_a_favor = [];
    ...
    //this.validator.limpiaInputRow(document.getElementById("pagos_moneda"));
    console.log("pagos_moneda");
  ...
  onSavePagoForCompra(form: NgForm): void {
    Swal.fire({
      title: this.translate.instant("swal_attenc"),
          ...
          this.docsOrdPayAnexos).subscribe(
            response => {
              ...
              if (response.status == 'success') {
                ...
                this.calculaTotalSaldo();
                this.relInterna.mensajeComprasPagoRegistrado("pago_realizado");
                this.showCajasRegistradas();
                this.getCuentasBancarias();
                this.getMonederosElectronicos();
                form.reset();
                ...
                this.formAddPagoNuevo.resetForm();
              }
