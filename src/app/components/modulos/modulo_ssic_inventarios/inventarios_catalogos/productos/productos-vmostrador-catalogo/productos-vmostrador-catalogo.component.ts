...
@Component({
  selector: 'app_productos_vmostrador_catalogo',
  standalone: false,
  
  templateUrl: './productos-vmostrador-catalogo.component.html',
  styleUrl: './productos-vmostrador-catalogo.component.css'
})
export class ProductosVmostradorCatalogoComponent implements OnInit, OnDestroy {
  loading = false;
  ...
  ngOnInit(): void {
    ...
    this.ver_productos_mostrador('hoy');
    this.buscList_prod = ['folio_prod','producto','sat_clave_code','ventanas','token_cat_productos','authorized','authorized_fecha','utilizado'];
  }

  getRespuestaRegistro(){
    this.relInterna.mensajeProdInvent$.subscribe(
      (mensaje:any) => {
        mensaje == "producto registrado" ? this.listar_productos_mostrador() : null;
      }
    );
  }
  ...
  getRespuestaMoveToPapelera(){
    this.relInterna.mensajeMoveToPepelera$.subscribe(
      (mensaje:any) => {
        mensaje == "registro eliminado" ? this.listar_productos_mostrador() : null;
      }
    );
  }
  ...
  getRespuestaRestaurarProd(){
    this.relInterna.mensajeRestaurarProd$.subscribe(
      (mensaje:any) => {
        mensaje == "registro restaurado" ? this.listar_productos_mostrador() : null;
      }
    );
  }
  listar_productos_mostrador(){
    this.ver_productos_mostrador(this.indicadorProductosList);
  }
  ver_productos_mostrador(filtro: 'hoy'|'esta_semana'|'este_mes'|'mes_anterior'|'otras_fechas'|'all_partidas') {
    ...
    if (filtro == 'otras_fechas') {
      var prod_mostrador_otras_fechas = document.getElementById("prod_mostrador_otras_fechas");
      if (this.rangoPeriodoProductos && this.rangoPeriodoProductos.length === 2) {
        ...
        if (dateInicio && dateFin) {
          ...
          if (validacionInicio && validacionFin) {
            ...
            this.validator.correctoInputRow(prod_mostrador_otras_fechas);
          } else {
            this.validator.errorInputRow(prod_mostrador_otras_fechas);
            return;
          }
        } else {
          this.validator.errorInputRow(prod_mostrador_otras_fechas);
    ...
    this._prodService.prodVentasMostradorLista(filtro,periodo_inicio,periodo_fin).pipe(takeUntil(this.destruir$)).subscribe({
  ...
  procesar_respuesta_producto(response: any){
    ...
    if (response.status === 'success') {
      this.listaProductosMostrador = response.listado;
      this.cd.detectChanges(); // Forzamos detección de cambios si es necesario
    } else {
      this.listaProductosMostrador = []; // O manejar mensaje de "sin datos"
    }
  }
  ...
  solicita_auth_producto(token_cat_productos:any){
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this._prodService.solicitarValidateProducto(token_cat_productos).subscribe(
          response => {
  ...
  verGeneralesProd(token_cat_productos:any){
    this.relInterna.mensajeVerGeneralesProd("ver producto mostra_vent",token_cat_productos);
  }
  ...
  btnDeleteProducto(token_cat_productos:any){
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this._prodService.moverPapeleraProducto(token_cat_productos).subscribe(
          response => {
            let translate_response = this.translate.instant(response.message);
            if (response.status == 'success') {
              this.listar_productos_mostrador();
