...
@Component({
  selector: 'app_compras_egr_comi_lista_conc',
  templateUrl: './compras_comisiones_lista_conc.component.html',
  standalone:false,
  styleUrls: [
    ...
    './compras_comisiones_lista_conc.component.css'
  ]
})
export class EgresosComisionesListaConcluidasComponent implements OnInit, OnDestroy {
  ...
  //lista
    comi_cconc_search:any = [];
    array_comisiones_concluidas:any = [];
    indicadorComiConc:'hoy'|'esta_semana'|'este_mes'|'mes_anterior'|'otras_fechas'|'all_partidas' = 'hoy';
    rangoPeriodoComiConc: Date[] | undefined;
  ...
  ngOnInit(): void {
    ...
    this.getRespuestaComisionConcluida();
    this.get_mes();
    this.comi_cconc_search = [
      'token_comision_main',
      'folio_comision',
      'comision_proyecto',
      'usuario_comision',
      ...
      'duracion',
      'recibe_dinero',
      'dinero_recibido','comision_moneda_name',
  ...
  getRespuestaComiReemSeccionModule() {
    this.relInterna.mensajeComiReemSeccionModule$.subscribe(
      (mensaje: any) => {
        if (mensaje == "seccion_comi_concluidas") {
          console.log(mensaje);
          if (this.array_comisiones_concluidas.length === 0) this.lista_comisiones_concluidas('hoy');
        }
      }
    );
  }
  ...
  getRespuestaComisionConcluida(){
    this.relInterna.mensajeEgresosComisionConcluida$.subscribe(
      (mensaje:any) => {
        if (mensaje == "comision_concluida") {
          this.lista_comisiones_concluidas(this.indicadorComiConc);
        }
      }
    );
  }
  ...
  //lista
    recargar_lista_concluidas() {
      this.lista_comisiones_concluidas(this.indicadorComiConc);
    }

    lista_comisiones_concluidas(filtro: 'hoy'|'esta_semana'|'este_mes'|'mes_anterior'|'otras_fechas'|'all_partidas') {
      this.indicadorComiConc = filtro;
      let periodo_inicio = '';
      let periodo_fin = '';
  
      if (filtro == 'otras_fechas') {
        var comi_conc_otras_fechas = document.getElementById("comi_conc_otras_fechas");
        if (this.rangoPeriodoComiConc && this.rangoPeriodoComiConc.length === 2) {
          const dateInicio = this.rangoPeriodoComiConc[0];
          const dateFin = this.rangoPeriodoComiConc[1];
          if (dateInicio && dateFin) {            
            ...
            if (validacionInicio && validacionFin) {
              ...
              this.validator.correctoInputRow(comi_conc_otras_fechas);
            } else {
              this.validator.errorInputRow(comi_conc_otras_fechas);
              return;
            }
          } else {
            this.validator.errorInputRow(comi_conc_otras_fechas);
      ...
      this.comi_serv.comision_listas_concluidas(filtro,periodo_inicio,periodo_fin).pipe(takeUntil(this.destruir$)).subscribe({
    ...
    private procesarRespuestaComi(response: any) {
      if (response.status === 'success') {
        ...
        this.array_comisiones_concluidas = [];
      }
    }
          ...
          this.comi_serv.egresos_comisiones_reabrir(token_comision).subscribe(
            response => {
              ...
              if (response.status == "success") {
                ...
                this.recargar_lista_concluidas();
    ...
    keyupComisionUpdate_empleado(comisionado_token:string){
      var comi_sionado_opcion_conc = document.getElementById("comi_sionado_opcion_conc");
      ...
      validacion && typeof comi_sionables_lista !== 'undefined' ? this.validator.correctoInputRow(comi_sionado_opcion_conc) : this.validator.errorInputRow(comi_sionado_opcion_conc);
    }
  ...
  //nuevo registro
    ...
    cleanFormRegistro(){
      ...
      this.validator.limpiaSelect(document.getElementById("comi_sionado_opcion_conc"));
