...
@Component({
  selector: 'app_compras_egr_comi_lista_general',
  templateUrl: './compras_comisiones_lista_general.component.html',
  standalone: false,
  styleUrls: [
    ...
    './compras_comisiones_lista_general.component.css'
  ]
})
export class EgresosComisionesListaGeneralComponent implements OnInit, OnDestroy {
  ...
  //lista
  comi_general_search: any = [];
  array_comisiones_general: any = [];
  indicadorComiList:'hoy'|'esta_semana'|'este_mes'|'mes_anterior'|'otras_fechas'|'all_partidas' = 'hoy';
  rangoPeriodoComiList: Date[] | undefined;
  //detalle
  ...
  ngOnInit(): void {
    ...
    this.getRespuestaComisionLGeneral();
    this.get_mes();
    this.comi_general_search = [
      'token_comision_main',
      'folio_comision',
      'comision_proyecto',
      'usuario_comision',
  ...
  getRespuestaComiReemSeccionModule() {
    this.relInterna.mensajeComiReemSeccionModule$.subscribe(
      (mensaje: any) => {
        if (mensaje == "seccion_comi_lista_general") {
          console.log(mensaje);
          if (this.array_comisiones_general.length === 0) this.lista_comisiones_general('hoy');
        }
      }
    );
  }
  ...
  getRespuestaComisionLGeneral() {
    this.relInterna.mensajeEgresosComisionLGeneral$.subscribe(
      (mensaje: any) => {
        if (mensaje == "comision_lista_general") {
          this.lista_comisiones_general(this.indicadorComiList);
        }
      }
    );
  }
  ...
  recargar_lista_comisiones() {
    this.lista_comisiones_general(this.indicadorComiList);
  }

  lista_comisiones_general(filtro: 'hoy'|'esta_semana'|'este_mes'|'mes_anterior'|'otras_fechas'|'all_partidas') {
    this.indicadorComiList = filtro;
    let periodo_inicio = '';
    let periodo_fin = '';

    if (filtro == 'otras_fechas') {
      var comi_gral_otras_fechas = document.getElementById("comi_gral_otras_fechas");
      if (this.rangoPeriodoComiList && this.rangoPeriodoComiList.length === 2) {
        const dateInicio = this.rangoPeriodoComiList[0];
        const dateFin = this.rangoPeriodoComiList[1];
        if (dateInicio && dateFin) {          
          ...
          if (validacionInicio && validacionFin) {
            ...
            this.validator.correctoInputRow(comi_gral_otras_fechas);
          } else {
            this.validator.errorInputRow(comi_gral_otras_fechas);
            return;
          }
        } else {
          this.validator.errorInputRow(comi_gral_otras_fechas);
    ...
    this.comi_serv.comisiones_lista_general(filtro,periodo_inicio,periodo_fin).pipe(takeUntil(this.destruir$)).subscribe({
  ...
  private procesarRespuestaComi(response: any) {
    if (response.status === 'success') {
      this.array_comisiones_general = response.comi_listado;
      console.log("this.array_comisiones_general");
      console.log(this.array_comisiones_general);
      this.cd.detectChanges();
    } else {
      this.array_comisiones_general = [];
    }
  }
        ...
        if (response.status == 'success') {
          ...
          this.arrayComisionDetalle.forEach((comi: any) => {
            ...
            if (comi.comision_relaciones_total == 0) {
              ...
              this.comisionado_tipo = comi.comisionado_tipo;
              this.comisionado_token = comi.comisionado_tkn;
              this.comisionado_nombre = comi.comisionado_name;

              this.formInfoRecibida.patchValue({ comisionado_token: comi.comisionado_tkn });
              this.formInfoRecibida.patchValue({ comisionado_nombre: comi.comisionado_name });
        ...
        this.comi_serv.egresos_comisiones_reabrir(token_comision).subscribe(
          response => {
            ...
            if (response.status == "success") {
              ...
              this.recargar_lista_comisiones();
  ...
  keyupComisionUpdate_empleado(comisionado_token: string) {
    var comi_sionado_opcion_gral = document.getElementById("comi_sionado_opcion_gral");
    ...
    validacion && typeof comi_sionables_lista !== 'undefined' ? this.validator.correctoInputRow(comi_sionado_opcion_gral) : this.validator.errorInputRow(comi_sionado_opcion_gral);
  }
  ...
  cleanFormRegistro() {
    ...
    this.validator.limpiaSelect(document.getElementById("comi_sionado_opcion_gral"));
