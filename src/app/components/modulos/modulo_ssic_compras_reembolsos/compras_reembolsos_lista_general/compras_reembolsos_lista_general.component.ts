...
@Component({
  selector: 'app_compras_egr_reembolsos_lista_general',
  templateUrl: './compras_reembolsos_lista_general.component.html',
  standalone: false,
  styleUrls: [
    ...
    './compras_reembolsos_lista_general.component.css',
  ]
})
export class EgresosReembolsosGeneralComponent implements OnInit, OnDestroy {
  public usuario: Usuarios;
  public identidad: any;

  reem_general_search: any = [];
  ...
  expandedRowsReembolsosGeneralReem: { [s: string]: boolean } = {};
  ...
  ngOnInit(): void {
    this.getRespuestaReembolsosLGeneral();
    this.ver_permisos_para_egresos();
    this.reembolsos_lista_general('hoy');

    this.reem_general_search = ['folio_reem', 'folio_solicitud', 'comision_folio', 'comision_proyecto', 'nombreEmiPers', 'company', 'fecha_gasto_html', 'ticket_gasto', 'pagado_a',
      ...
      'importe_requ_info_sali_format', 'observaciones', 'token_solicitud_reem', 'terminado', 'autorizacion_vh', 'fecha_registro_auth_vh', 'max_auth_egr',
      'fecha_registro_auth_egr', 'hora_registro_auth_egr'];
  ...
  getRespuestaReembolsosLGeneral() {
    this.relInterna.mensajeEgresosReembolsoLGeneral$
    .pipe(takeUntil(this.destruir$)).subscribe(
      (mensaje: any) => {
        if (mensaje == "reembolso_compra_movimiento") {
          this.reembolsos_lista_general(this.indicadorReemList);
        }
      }
    );
  }
  ...
  reembolsos_lista_general(filtro: 'hoy'|'esta_semana'|'este_mes'|'mes_anterior'|'otras_fechas'|'all_partidas') {
    this.indicadorReemList = filtro;
    let periodo_inicio = '';
    let periodo_fin = '';

    if (filtro == 'otras_fechas') {
      var reem_gral_otras_fechas = document.getElementById("reem_gral_otras_fechas");
      if (this.rangoPeriodoReemList && this.rangoPeriodoReemList.length === 2) {
        ...
        if (dateInicio && dateFin) {
          ...
          if (validacionInicio && validacionFin) {
            ...
            this.validator.correctoInputRow(reem_gral_otras_fechas);
          } else {
            this.validator.errorInputRow(reem_gral_otras_fechas);
            return;
          }
        } else {
          this.validator.errorInputRow(reem_gral_otras_fechas);
    ...
    this.reem_serv.list_reembolsos_egr_general(filtro,periodo_inicio,periodo_fin).pipe(takeUntil(this.destruir$)).subscribe({
  ...
  disabled_btn_auth_simple(token_solicitud_reem: any) {
    //console.log(token_solicitud_reem);
    const reem_soli = this.reembolsos_list_general.find((reem: any) => reem.token_solicitud_reem === token_solicitud_reem);
    const condicion = reem_soli.terminado || reem_soli.autorizacion_vh == 'D' || reem_soli.max_auth_egr || reem_soli.autorizacion_egr;
    return condicion;
  }

  disabled_btn_cancelar_reembolso_simple(token_solicitud_reem: any) {
    const reem_soli = this.reembolsos_list_general.find((reem: any) => reem.token_solicitud_reem === token_solicitud_reem);
    const condicion = reem_soli.terminado || reem_soli.autorizacion_vh == 'D' || reem_soli.compras_vinculadas.length > 0;// || !reem_soli.max_auth_egr || !reem_soli.autorizacion_egr 
    return condicion;
  }

  authorizarReem(token_reem: any, token_solicitud_reem: any, autorizacion_egr: any) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.reem_serv.reembolso_egr_auth(token_reem, token_solicitud_reem, autorizacion_egr).pipe(takeUntil(this.destruir$)).subscribe(
          response => {
            let translate_response = this.translate.instant(response.message);
            if (response.status == 'success') {
              this.recargar_lista_reembolsos();
              this.relInterna.mensajeEgresosReembolsosAutorizados("reembolso_autorizado");
              setTimeout(function () {
                Swal.fire({
                  position: 'center',
                  icon: 'success',
  ...
  keyupObservaReem(event: any, token_solicitud_reem: any) {
    const reem_soli = this.reembolsos_list_general.find((reem: any) => reem.token_solicitud_reem === token_solicitud_reem);
    const validacion = event.value != "" && this.validator.filtroAlfaNumerico(event.value) && event.value.length >= 4;
    reem_soli.comments_auth_egr_write = validacion ? event.value : "";
    console.log(reem_soli.comments_auth_egr_write);
    validacion ? this.validator.correctoInputRow(event) : this.validator.errorInputRow(event);
  }

  graba_observaciones_reembolso(token_solicitud_reem: any) {
    Swal.fire({
      title: this.translate.instant("swal_attenc"),
      text: this.translate.instant("swal_update"),
      icon: 'warning',
      ...
      if (result.isConfirmed) {
        const reem_soli = this.reembolsos_list_general.find((reem: any) => reem.token_solicitud_reem === token_solicitud_reem);
        this.reem_serv.reembolso_egr_observaciones_auth(reem_soli.token_reem, token_solicitud_reem, reem_soli.comments_auth_egr_write).pipe(takeUntil(this.destruir$)).subscribe(
          response => {
            let translate_response = this.translate.instant(response.message);
            this.primeAlerts.add({ severity: response.status, summary: 'SOS-México informa: ', detail: translate_response });
            if (response.status == 'success') {
              this.recargar_lista_reembolsos();
              this.relInterna.mensajeEgresosReembolsosAutorizados("reembolso_autorizado");
            }
  ...
  disabled_btn_auth_vinc(token_solicitud_reem: any) {
    const reem_soli = this.reembolsos_list_general.find((reem: any) => reem.token_solicitud_reem === token_solicitud_reem);
  ...
  ver_solicitudes_cancelacion(lreem: any,filtro: 'hoy'|'esta_semana'|'este_mes'|'mes_anterior'|'otras_fechas'|'all_partidas') {
    ...
    if (filtro == 'otras_fechas') {
      var reem_gral_vinc_buy_otras_fechas = document.getElementById("reem_gral_vinc_buy_otras_fechas");
      if (this.rangoPeriodoBuyVinc && this.rangoPeriodoBuyVinc.length === 2) {
        ...
        if (dateInicio && dateFin) {
          ...
          if (validacionInicio && validacionFin) {
            periodo_inicio = dateInicio.toISOString().split('T')[0];
            periodo_fin = dateFin.toISOString().split('T')[0];
            this.validator.correctoInputRow(reem_gral_vinc_buy_otras_fechas);
          } else {
            this.validator.errorInputRow(reem_gral_vinc_buy_otras_fechas);
            return;
          }
        } else {
          this.validator.errorInputRow(reem_gral_vinc_buy_otras_fechas);
  ...
  verHistorial(lreem: any) {
    this.reembolsoPerfilAutorizaciones = [];
    this.reembolsoPerfilAutorizaciones.push(lreem);
    this.folio_reembolso_main = lreem.folio_reem;
    this.folio_reembolso_solicitud = lreem.folio_solicitud;
    this.viewModalListadoDeAutorizaciones = true;
  }
  ...
  alertaViewDocs() {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        this.userServ.user_solicitar_permiso_ver_docs(empresaToken, this.identidad.user_token, "eegr")
          .pipe(takeUntil(this.destruir$)).subscribe(
            response => {
              let translate_response = this.translate.instant(response.message);
              if (response.status == "success") {
  ...
  keyupObservaReemComprasUNVincular(event: any, token_solicitud_reem: any) {
    const canc_soli = this.reembolsos_list_general.find((reem: any) => reem.token_solicitud_reem === token_solicitud_reem);
  ...
  solicitaDesvuinculacionReembolso(reem: any, token_compras: any) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.reem_serv.reembolso_cancela_vinc_compras(reem.token_reem,reem.token_solicitud_reem,token_compras,reem.soli_cancela_vinc_comentarios).pipe(takeUntil(this.destruir$)).subscribe(
          response => {
            let translate_response = this.translate.instant(response.message);
            if (response.status == 'success') {
              this.recargar_lista_reembolsos();
              this.relInterna.mensajeEgresosReembolsosAutorizados("reembolso_autorizado");
              reem.soli_cancela_vinc_comentarios = "";
              this.validator.limpiaInputRow(document.getElementById("aclaraciones_solicitud_cancelacion"));
              setTimeout(function () {
                Swal.fire({
                  position: 'center',
                  icon: 'success',
  ...
  vincularReembolsoACompraYPagarAAcreedor(token_reem: any, token_solicitud_reem: any, token_compras: any, observaciones: any) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        const compra = this.reem_soli_compras_vincular.find((buy: any) => buy.token_compras === token_compras);
        //compra.fecha_contabilizacion_html = validacion_xml ? event.value : '';
        this.reem_serv.reembolso_egr_auth_pagar_a_acreedor(token_reem, token_solicitud_reem, token_compras, compra.fecha_contabilizacion_html, observaciones).pipe(takeUntil(this.destruir$)).subscribe(
          response => {
            if (response.status == 'success') {
              let translate_response = this.translate.instant(response.message);
              if (response.status == 'success') {
