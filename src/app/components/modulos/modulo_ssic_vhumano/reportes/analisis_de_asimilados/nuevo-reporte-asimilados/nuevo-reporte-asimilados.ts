...
@Component({
  selector: 'vhumano_reportes_asimilados_nuevo_reporte',
  standalone: false,
  templateUrl: './nuevo-reporte-asimilados.html',
  styleUrls: [
    ...
    '../../../../../../styles/totales.css',
    '../../../vhumano.css',
    './nuevo-reporte-asimilados.css'
  ]
})
export class NuevoReporteAsimilados implements OnInit, OnDestroy {
  private destroy$ = new Subject<void>();
  public viewNewAsimiladoForm: boolean = true;
  public modelAsimilados: asimiladosModelo;
  asim_moneda_opcion = null;
  ...
  getRespuestaRegistroProveed() {
    this.proveedorRegistroSubscription = this.relInterna.mensajeProveedorEgresos$.subscribe(
      (mensaje: any) => {
        mensaje == "registro aprobado" ? this.revisa_receptor_proveedor_registrado() : null;
      }
    );
  }
  ...
  procesaCuerpoCFDI(xmlNode: any, childNodes: any) {
    ...
    this.dataCFDI_comprobante_obj = {
      ...
      Descuento: xmlNode.getAttribute('Descuento') || '0.00',
  ...
  obtenComplemento_Nomina(nodo_complemento: any) {
    nodo_complemento.forEach((child: any) => {
      ...
      nodo_nomina12_Nomina.forEach((mino: any) => {
        ...
        this.nominaForm.patchValue({ periodo_de_pago: [mes_periodo_inicio, mes_periodo_fin] });

        this.modelAsimilados.fecha_pago = mino.getAttribute("FechaPago") || '';
        this.nominaForm.patchValue({ fecha_de_pago: mino.getAttribute("FechaPago") });
        
        this.nominaForm.patchValue({ mon_name: this.dataCFDI_comprobante_Moneda });

        this.modelAsimilados.dias_pagados = mino.getAttribute("NumDiasPagados") || '';
        this.nominaForm.patchValue({ dias_pagados: mino.getAttribute("NumDiasPagados") });

        this.modelAsimilados.total_percepciones = mino.getAttribute("TotalPercepciones") || '';
        this.nominaForm.patchValue({ total_percepciones: mino.getAttribute("TotalPercepciones") });

        this.modelAsimilados.total_deducciones = mino.getAttribute("TotalDeducciones") || '';
        this.nominaForm.patchValue({ total_deducciones: mino.getAttribute("TotalDeducciones") });
  ...
  get validaRegistroAsimiladosReport():Boolean{
    const OKFechaCont = this.asimilado_fecha_contabilizacion != "" && this.validator.filtroFecha(this.asimilado_fecha_contabilizacion);
    const OKReceptor = this.asimilado_token != '';
    const OKPeriodoInicio = this.modelAsimilados.periodo_inicio != '' && this.validator.filtroFecha(this.modelAsimilados.periodo_inicio);
    const OKPeriodoFin = this.modelAsimilados.periodo_fin != '' && this.validator.filtroFecha(this.modelAsimilados.periodo_fin);
    const OKFechaPago = this.modelAsimilados.fecha_pago != '' && this.validator.filtroFecha(this.modelAsimilados.fecha_pago);
    const OKMoneda = this.modelAsimilados.moneda_code != '' && this.validator.filtroAlfaNumerico(this.modelAsimilados.moneda_code);
    const OKDiasPagados = this.modelAsimilados.dias_pagados != '' && this.validator.filtroAlfaNumerico(this.modelAsimilados.dias_pagados);
    const OKTotalPercepciones = this.modelAsimilados.total_percepciones > 0 && this.validator.filtroNum(this.modelAsimilados.total_percepciones);
    const OKTotalDeducciones = this.modelAsimilados.total_deducciones > 0 && this.validator.filtroNum(this.modelAsimilados.total_deducciones);
    const OKObservaciones = this.asimilado_observaciones != '' && this.validator.filtroAlfaNumerico(this.asimilado_observaciones);
    const OKxmlArticulos = this.selectvalidatexmlArticulos == true;
    const OKDocuments = this.AsimiladosAnexosNames.length > 0;

    return this.imagenEvidenciaXml && this.imagenEvidenciaPdf && OKFechaCont && OKReceptor && OKPeriodoInicio && OKPeriodoFin && OKFechaPago && OKMoneda && 
      OKTotalPercepciones && OKTotalDeducciones && OKObservaciones && this.selectvalidatexmlArticulos;
  }

  registraNominaReporte(form: { reset: () => void; }):void{
    Swal.fire({
      title: this.translate.instant("swal_attenc"),
      text: this.translate.instant("swal_insert"),
      icon: 'warning',
      ...
      if (result.isConfirmed) {
        ...
        this.asimService.registra_reporte_asimilados(
