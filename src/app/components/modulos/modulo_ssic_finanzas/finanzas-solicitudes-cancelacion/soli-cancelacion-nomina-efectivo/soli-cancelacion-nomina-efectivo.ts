...
@Component({
  selector: 'fnzs_solicitud_cancelacion_nomina_efectivo',
  standalone: false,
  templateUrl: './soli-cancelacion-nomina-efectivo.html',
  styleUrls: [
    ...
    '../../../../../styles/switches.css',
    '../../finanzas.css',
    './soli-cancelacion-nomina-efectivo.css',
  ]
})
export class SoliCancelacionNominaEfectivo implements OnInit{
  public dataSoliCancelacionEfectivoNomina:any = [];
  desglose_solicitud_cancelacion:any = [];
  desgloseSoliCancelacionDispersionEfectivo: any = [];
  ...
  @Input() set SoliCancelacion(value: string) {
    if (value) {
      this.dataSoliCancelacionEfectivoNomina = value;
      this.solicitud_de_cancelacion_nomina_efectivo(this.dataSoliCancelacionEfectivoNomina);
    }
  }
  ...
  solicitud_de_cancelacion_nomina_efectivo(canc_soli: any) {
    ...
    this.ordenDisper.solicitud_cancelacion_orden_dispersion_nomina_efectivo(canc_soli.cancel_soli_token,canc_soli.doc_anterior_token).subscribe(
      response => {
        if (response.status == 'success') {
          this.desgloseSoliCancelacionDispersionEfectivo = response.orden_pago;
          console.log(this.desgloseSoliCancelacionDispersionEfectivo);
        }
      },
      error => {
        console.log(error);
  ...
  confirmaCancelacionOrdenDispersionNominaEfectivo(cancel_soli_token: any, f_contabilizacion: any, comentarios_confirma_cancelacion: any) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.ordenDisper.confirmar_cancelacion_orden_dispersion_nomina_efectivo(cancel_soli_token, f_contabilizacion, comentarios_confirma_cancelacion).subscribe(
          response => {
