...
@Component({
  selector: 'fnzs_solicitud_cancelacion_orden_de_pago',
  standalone: false,
  templateUrl: './soli-cancelacion-orden-de-pago.html',
  styleUrls: [
    ...
    '../../../../../styles/switches.css',
    '../../finanzas.css',
    './soli-cancelacion-orden-de-pago.css',
  ]
})
export class SoliCancelacionOrdenDePago implements OnInit{
  public dataSoliCancelacionOrdenPago:any = [];
  desglose_solicitud_cancelacion:any = [];
  desgloseSoliCancelacionOrdenPago: any = [];
  ...
  @Input() set SoliCancelacion(value: string) {
    if (value) {
      this.dataSoliCancelacionOrdenPago = value;
      this.solicitud_de_cancelacion_orden_pago(this.dataSoliCancelacionOrdenPago);
    }
  }
  ...
  solicitud_de_cancelacion_orden_pago(canc_soli: any) {
    this.desglose_solicitud_cancelacion = [];
    
    this.ordenPago.solicitud_cancelacion_orden_pago(canc_soli.cancel_soli_token,canc_soli.doc_anterior_token).subscribe(
      response => {
        if (response.status == 'success') {
          this.desglose_solicitud_cancelacion.push(canc_soli);
          this.desgloseSoliCancelacionOrdenPago = response.orden_pago;
          console.log(this.desgloseSoliCancelacionOrdenPago);
        }
      },
      error => {
        console.log(error);
  ...
  confirmaCancelacionOrdenPago(cancel_soli_token: any, f_contabilizacion: any, comentarios_confirma_cancelacion: any) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.ordenPago.confirma_cancelacion_orden_pago(cancel_soli_token, f_contabilizacion, comentarios_confirma_cancelacion).subscribe(
          response => {
