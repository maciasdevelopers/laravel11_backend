...
@Component({
  selector: 'fnzs_pago_nomina_efectivo_form_component',
  standalone: false,
  templateUrl: './pago-nomina-efectivo-component.html',
  styleUrls: [
    ...
    '../../../../../styles/switches.css',
    '../../finanzas.css',
    './pago-nomina-efectivo-component.css',
  ]
})
export class PagoNominaEfectivoComponent implements OnInit, OnChanges, OnDestroy{
  @Input() orden_de_pago!: string;
  @Input() nominas_periodos!: string;
  ...
  ngOnChanges(changes: SimpleChanges): void {
    ...
    const cambioPeriodos = changes['nominas_periodos'];

    if (cambioOrden || cambioPeriodos) {
      if (this.orden_de_pago && this.nominas_periodos) {
        this.orden_pago_nomina_efectivo_form = false;
        this.ordenPago.pagoNominaDesglose(this.orden_de_pago, this.nominas_periodos).subscribe(
          response => {
            if (response.status == 'success') {
              console.log(response);
              this.orden_pago_nomina_efectivo_form = true;
              this.limpiaSeccionPagos();
              this.ordenes_pago_listaProceso = response.desglose;
              console.log(response.desglose)
  ...
  ordenPagoAdeudoNominaFormat() {
    ...
    this.ordenes_pago_listaProceso.forEach((row: any) => {
      importe_autorizado += parseFloat(row.nomina_total_efectivo_simple) || 0;
  ...
  calculaTotalPagandoNominaFormat() {
    var importe_autorizado = 0;
    var moneda_autorizada = "";
    var moneda_autorizada_decimales = "";
  ...
  calculaTotalNominaSaldo(): string {
  ...
  aumentaImporteAll() {
    ...
    if (this.ordenes_pago_listaProceso.length == 1) {
      ...
      ord.debe_format = numeral(ord.debe_simple).format('0,0.' + '0'.repeat(parseInt(this.pay_order_moneda_decimales.toString())))+" "+this.pay_order_moneda_code;
    }
  }
  ...
  keyupOrdenPImportePorPagar(event: any, lista_proceso: any) {
    const validacion = event.value != '' && this.validator.filtroNum(event.value) && parseFloat(event.value) <= parseFloat(lista_proceso.importe_restante);
    lista_proceso.importe_por_pagar = validacion ? event.value : '0.00';
    lista_proceso.debe_simple = validacion ? parseFloat(lista_proceso.importe_restante) - parseFloat(lista_proceso.importe_por_pagar) : lista_proceso.importe_restante;
    const new_format_debe = numeral(lista_proceso.debe_simple).format('0,0.' + '0'.repeat(parseInt(this.pay_order_moneda_decimales.toString())))+" "+this.pay_order_moneda_code;
    const rest_format_debe = numeral(lista_proceso.importe_restante).format('0,0.' + '0'.repeat(parseInt(this.pay_order_moneda_decimales.toString())))+" "+this.pay_order_moneda_code;
    lista_proceso.debe_format = validacion ? new_format_debe : rest_format_debe;
    validacion ? this.validator.correctoInputRow(event) : this.validator.errorInputRow(event);
  }
  ...
  get validate_pago_for_nomina_efectivo(): Boolean {
    ...
    if (pagar_list.length > 0) {
      this.ordenes_pago_listaProceso.forEach((row: any) => {
        importe_registrado_por_pagar += parseFloat(row.importe_por_pagar);
      });
    }
    var importe_actualizado_por_pagar = parseFloat(numeral(importe_registrado_por_pagar).format('0.' + '0'.repeat(parseInt(this.pay_order_moneda_decimales.toString()))));
    const validacion_pagando = pagar_list.length > 0 && importe_actualizado_por_pagar > 0 && importe_actualizado_por_pagar == this.pay_order_importe;
  ...
  onSavePagoNominaEfectivo(form: NgForm): void {
    Swal.fire({
      title: this.translate.instant("swal_attenc"),
      ...
      if (result.isConfirmed) {
        ...
        const cuent_list = this.listaCuentasBancarias.filter((row: any) => row.select_for_pagos === true);
        const moned_list = this.listaCuentasMonederoElectronico.filter((row: any) => row.select_for_pagos === true);
        const trabajadores_pagar = this.ordenes_pago_listaProceso.filter((row: any) => row.importe_por_pagar != "0.00");
        this.ordenPago.confirmaDispersionNomina(
          this.orden_de_pago,
          this.nominas_periodos,
          ...
          this.docsOrdPayAnexos).subscribe(
            response => {
              ...
              if (response.status == 'success') {
                ...
                form.resetForm();
                this.limpiaSeccionPagos();
                this.formRegistroPagoNominaEF.resetForm();
